      ...
      ol
        li(v-for="(item,i) in groups_nodes" :key="i" @dragstart="e=>dragstart(e,i)" @dragend="e=>dragend(e,i)" @dragover="e=>dragover(e,i)" :draggable="true")
          template(v-if="item.children!== undefined")
            Group(:item="item" @changeMode="changeGroupMode(item)" @mousedown.stop="mouseDown(item, 'group')" @mouseup.stop="mouseUp")
              template(v-for="(node,j) in item.children" :key="j")
                Node(:node="node" @changeMode="changeNodeMode(node)" @mousedown.stop="mouseDown(node, 'node')" @mouseup.stop="mouseUp")
          template(v-else)
            Node(:node="item.info" @changeMode="changeNodeMode(item.info)" @mousedown.stop="mouseDown(item.info,'node')" @mouseup.stop="mouseUp")
